...
and `final`. The purpose of these latter arguments is explained by the following
...
- If `ord = ForwardFilter`, then `J` is the subset of all indices `j` such that `A[j]` and
  `B[j-i]` are in-bounds. This is the natural ordering to implement discrete correlations.

- If `ord = ReverseFilter`, then `J` is the subset of all indices `j` such that `A[j]` and
  `B[i-j]` are in-bounds. This is the natural ordering to implement discrete convolutions.
...
`ForwardFilter` or `ReverseFilter`.
