...
"""
    erode(A, [ord=FORWARD_FILTER,] B=3; slow=false) -> Amin
...
"""
    erode!(Amin, A, [ord=FORWARD_FILTER,] B=3; slow=false) -> Amin
...
"""
    dilate(A, [ord=FORWARD_FILTER,] B=3; slow=false) -> Amax
...
"""
    dilate!(Amax, A, [ord=FORWARD_FILTER,] B=3; slow=false) -> Amax
...
for (f, op) in ((:erode, :min), (:dilate, :max))
    ...
    @eval begin
        ...
        function $f!(dst::AbstractArray{<:Any,N},
                     ...
                     kwds...) where {N}
            return $f!(dst, A, FORWARD_FILTER, B; kwds...)
        end
...
function localextrema!(Amin::AbstractArray{<:Any,N},
    ...
    return localextrema!(Amin, Amax, A, FORWARD_FILTER, B)
end
...
"""
    closing(A, [ord=FORWARD_FILTER,] B=3; slow=false) -> dst
...
"""
    closing!(dst, wrk, A, [ord=FORWARD_FILTER,] B=3; slow=false) -> dst
...
"""
    opening(A, [ord=FORWARD_FILTER,] B=3; slow=false) -> dst
...
"""
    opening!(dst, wrk, A, [ord=FORWARD_FILTER,] B=3; slow=false) -> dst
...
for f in (:closing, :opening)
    ...
    @eval begin
        ...
        function $f!(dst::AbstractArray{<:Any,N},
            ...
            $f!(dst, wrk, A, FORWARD_FILTER, B; kwds...)
        end
        function $f!(dst::AbstractArray{<:Any,N},
                     wrk::AbstractArray{<:Any,N},
                     A::AbstractArray{<:Any,N},
...
"""
    top_hat(A, [B_ord=FORWARD_FILTER,] B=3 [, [C_ord=B_ord,] C]; slow=false) -> dst
...
"""
    LocalFilters.top_hat!(dst, wrk, A, [ord=FORWARD_FILTER,] B=3; slow=false) -> dst
...
"""
    bottom_hat(A, [B_ord=FORWARD_FILTER,] B=3 [, [C_ord=B_ord,] C]; slow=false) -> dst
...
"""
    LocalFilters.bottom_hat!(dst, wrk, A, [ord=FORWARD_FILTER,] B=3; slow=false) -> dst
...
for (f, pf) in ((:top_hat,    :closing),
    ...
    @eval begin
        # Provide default ordering and default structuring element.
        $f(A::AbstractArray, B=3; kwds...) = $f(A, FORWARD_FILTER, B; kwds...)
        $f(A::AbstractArray, B, C; kwds...) = $f(A, FORWARD_FILTER, B, C; kwds...)
        ...
        function $f!(dst::AbstractArray{<:Any,N},
                     ...
                     A::AbstractArray{<:Any,N},
                     B::Union{Window{N},AbstractArray{<:Any,N}} = 3; kwds...) where {N}
            return $f!(dst, wrk, A, FORWARD_FILTER, B; kwds...)
        end
